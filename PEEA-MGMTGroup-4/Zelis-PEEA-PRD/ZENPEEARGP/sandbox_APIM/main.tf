 ...
 module "apim" {
   ...
   source = "git::https://github.com/ZelisEnt/zelis_TF_PAAS_Modules.git//Modules//azurerm_api_management_api?ref=feature/integrate_network_restriction_n_health_check"
   #?ref=feature"
              providers = {
               aws = aws,
               azurerm = azurerm,
   ...
   tags = merge(
    ...
    module "app-kv" {
  source = "git::https://github.com/ZelisEnt/zelis_TF_PAAS_Modules.git//Modules//azurerm-key-vault?ref=feature/integrate_network_restriction_n_health_check"

  name = "zenpeasbxapimkvp01"
  env = var.env
  resource_group_name = var.resource_group_name
  location            = var.location
