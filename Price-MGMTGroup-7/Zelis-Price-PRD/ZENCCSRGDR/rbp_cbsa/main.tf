data "azurerm_storage_account" "strg" {
...
module "win-fun-app" {
  ...
  storage_account_name                = data.azurerm_storage_account.strg.name
  storage_account_id                  = data.azurerm_storage_account.strg.id
  resource_group_name                 = var.resource_group_name
  location                            = var.location
  ip_restriction = {
    "LZ Subnet" = {
...
module "CustomDNS_2" {
  ...
  zone_name                            = var.zone_name
  host_name                            = "rbp-cbsalookup-prod.zelis.com"
  record_type                          = var.record_type
  custom_domain_verification_id        = var.custom_domain_verification_id
  application_name                     = var.application_name
  resource_group_name                  = var.resource_group_name
